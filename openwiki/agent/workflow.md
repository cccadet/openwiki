...
- **openai**: `new ChatOpenAI({ apiKey, configuration: { baseURL? }, model })` — uses `OPENAI_BASE_URL` from the environment when set, otherwise calls the default OpenAI endpoint.
- **baseten / fireworks**: `new ChatOpenAI({ apiKey, configuration: { baseURL? }, model })` — OpenAI-compatible clients using the provider's custom base URL from `PROVIDER_CONFIGS`.
